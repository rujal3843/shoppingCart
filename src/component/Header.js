...
const Header = () => {
  ...
  return (
    <div className='w-full h-[100px] flex bg-[#FCBB04] text-white justify-between items-center px-4'>
      ...
      {/* mobile */}
      <ul className={!navi ? 'hidden' : 'absolute top-0 left-0 w-full h-screen bg-[#0a192f] flex flex-col justify-center items-center'}>
  <li className="py-2 text-[25px]">
    <NavLink onClick={handleclick} to="/" className="text-xl">
      Home
    </NavLink>
  </li>
  <li className="relative group">
    <NavLink to="#" className="hover:bg-pink-500">
      Category
    </NavLink>
    <ul className="absolute z-50 opacity-0 pointer-events-none transition duration-300 ease-in-out group-hover:opacity-100 group-hover:pointer-events-auto w-max bg-[#5C4434]">
      <li>
        <NavLink onClick={handleclick} to="/cate/electronics" className="hover:bg-pink-500">
          electronics
        </NavLink>
      </li>
      <li>
        <NavLink onClick={handleclick} to="/cate/jewelery" className="hover:bg-pink-500">
          jewelery
        </NavLink>
      </li>
      <li>
        <NavLink onClick={handleclick} to="/cate/men's clothing" className="hover:bg-pink-500">
          men's clothing
        </NavLink>
      </li>
      <li>
        <NavLink onClick={handleclick} to="/cate/women's clothing" className="hover:bg-pink-500">
          women's clothing
        </NavLink>
      </li>
    </ul>
  ...
  <div className="flex gap-6 items-center justify-between md:w-full">
    {!user && (
      <NavLink
        onClick={handleclick}
        to="/login"
        className="text-md text-black bg-white border py-2 px-6 font-bold hover:border-transparent rounded text-[25px]"
      >
        Login
      </NavLink>
    )}
